...
// --- CONFIGURATION ---
const IG_DOC_URL = "https://instagram.com/your_doc_handle";
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-6 relative overflow-hidden">
      ...
      <div className="max-w-xl w-full z-10 bg-zinc-950 border border-zinc-800 p-8 shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-6 text-blue-500">
          ...
          <span className="tracking-widest uppercase text-sm font-bold">Terminal / DOC.OS</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="flex items-center border-b-2 border-zinc-700 focus-within:border-blue-500 transition-colors pb-2">
            ...
            <input
              ...
              maxLength={10}
            />
            {error ? <Lock className="w-5 h-5 text-red-500" /> : <Unlock className="w-5 h-5 text-zinc-500" />}
          </div>
...
const ProphecyTab = () => (
  <div className="space-y-12 animate-in fade-in duration-700">
    <header className="border-b border-zinc-800 pb-6">
      <h2 className="text-sm text-blue-500 tracking-[0.4em] mb-2 uppercase">File: A.R_I18N</h2>
      <h1 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter">The Prophecy</h1>
    </header>
    ...
    <article className="prose prose-invert prose-p:text-zinc-400 prose-p:leading-loose prose-p:font-mono max-w-none">
      ...
      <div className="grid md:grid-cols-2 gap-12 mt-12">
        ...
        <div className="space-y-6">
          ...
          <p className="text-rose-500 font-bold">Beyond certainty. Beyond return. The way back was lost.</p>
        </div>
      </div>
      
      <div className="mt-16 p-8 border border-zinc-800 bg-zinc-950 text-center space-y-4">
        <p className="uppercase tracking-widest text-sm text-zinc-500">Transmission End</p>
        <p className="text-2xl font-black text-white tracking-tighter">YOU'VE GOT GREATNESS INSIDE YOU.</p>
      </div>
    </article>
  </div>
);
...
const ArchivesTab = () => {
  const panels = [
    { id: 1, title: "01: AWAKENING", text: "The moment before the shift.", type: "comic", link: IG_DOC_URL },
    { id: 2, title: "02: THE FRACTURE", text: "Reality begins to distort.", type: "art", link: IG_DOC_URL },
    { id: 3, title: "03: UNREST", text: "Is intensity a gift or a burden?", type: "quote", link: IG_TOD_URL },
    { id: 4, title: "04: THE ANOMALY", text: "A glitch in the perfect system.", type: "comic", link: IG_DOC_URL },
    { id: 5, title: "05: SEEKER", text: "Standing at the edge of chaos.", type: "art", link: IG_DOC_URL },
    { id: 6, title: "06: NO RETURN", text: "The way back was lost.", type: "comic", link: IG_DOC_URL },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:justify-between md:items-end border-b border-zinc-800 pb-6 gap-4">
        <div>
          <h2 className="text-sm text-blue-500 tracking-[0.4em] mb-2 uppercase">Database: Visual</h2>
          <h1 className="text-3xl font-black text-white uppercase tracking-tighter">Fragment Archives</h1>
        </div>
        <div className="flex flex-col items-end gap-2">
          <div className="text-xs text-zinc-500 tracking-widest">[ 9-GRID SYNC ACTIVE ]</div>
          <a href={IG_DOC_URL} target="_blank" rel="noreferrer" className="text-xs font-bold bg-blue-500/10 text-blue-400 border border-blue-500/30 px-3 py-1 uppercase hover:bg-blue-500 hover:text-black transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {panels.map((panel) => (
          ...
          >
            ...
            <div className={`absolute inset-0 opacity-20 transition-opacity duration-500 group-hover:opacity-10 
              ${panel.type === 'comic' ? 'bg-blue-600' : panel.type === 'art' ? 'bg-rose-600' : 'bg-zinc-600'}`} 
            />
            ...
            <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
              <div className="flex justify-between items-start">
                <div className="text-xs font-bold text-zinc-500 tracking-widest">{panel.title}</div>
              ...
              <div className="translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-out">
                ...
                <p className="text-sm font-bold text-white uppercase">{panel.text}</p>
                <p className="text-xs text-blue-400 mt-2 tracking-widest">DECODE ON INSTAGRAM →</p>
              </div>
            </div>
        ...
        {[7, 8, 9].map((lockedId) => (
          <div key={lockedId} className="aspect-square bg-black border border-zinc-900 flex items-center justify-center flex-col gap-3 group relative overflow-hidden">
            ...
            <span className="text-xs tracking-widest text-zinc-700 text-center px-4">ENCRYPTED<br/><span className="text-[10px] text-zinc-800">AWAITING IG TRANSMISSION</span></span>
          </div>
        ))}
      </div>
    </div>
...
const SeekersTab = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="border-b border-zinc-800 pb-6">
        <h2 className="text-sm text-green-500 tracking-[0.4em] mb-2 uppercase">Comm-Link: Active</h2>
        <h1 className="text-3xl font-black text-white uppercase tracking-tighter">Seeker Transmissions</h1>
      </header>

      <div className="bg-zinc-900/50 border border-zinc-800 p-6">
        <div className="space-y-6">
          {logs.map((log) => (
            <div key={log.id} className="border-b border-zinc-800/50 pb-6 last:border-0 last:pb-0">
              <div className="flex items-center gap-3 mb-2">
                <Zap className="w-4 h-4 text-green-500" />
                <span className="text-xs font-bold text-white tracking-widest">{log.user}</span>
        ...
        <div className="mt-8 pt-6 border-t border-zinc-800 relative">
          ...
          <div className="flex gap-4">
            <input 
              ...
              disabled
            />
            <button disabled className="bg-green-500/10 text-green-500 border border-green-500/30 px-6 py-3 text-sm font-bold tracking-widest uppercase hover:bg-green-500/20 transition-colors">
              Transmit
            </button>
          </div>
          <p className="text-xs text-rose-500 mt-2 text-right">TRANSMISSION LOCKED UNTIL DROP 1 LAUNCH</p>
        </div>
      </div>
    </div>
  );
